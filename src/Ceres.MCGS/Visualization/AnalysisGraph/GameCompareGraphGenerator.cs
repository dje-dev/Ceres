...
public class GameCompareGraphGenerator
{
  ...
  /// <summary>
  /// Generates SVG with game comparison details, returning FN of generated file.
  /// </summary>
  ...
  void GenSubgraph(List<PGNGame> games)
  {
    ...
    foreach (PGNCommonMoves.PGNCommonMoveSequence thisSet in PGNCommonMoves.PGNCommonMoveSequences(null, games, 0))
    {
      ...
      if (stack.Count > 0)
      {
        // Start a new set.
        WriteIndent(thisSet.depth);
        PGNCommonMoves.PGNCommonMoveSequence lastSet = stack.Peek();
    ...
    while (stack.Count > 0)
    {
      PGNCommonMoves.PGNCommonMoveSequence thisSet = stack.Pop();
      for (int i = 0; i < 2 + thisSet.depth * 2; i++)
      {
        dot.Append(" ");
      }
      dot.AppendLine("}");
  ...
  string PosNodeInfo(Position pos, MGMove mgMove, string firstLine, string[] extraLines)
  {
