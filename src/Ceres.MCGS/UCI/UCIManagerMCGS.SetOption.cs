#region License notice

/*
  This file is part of the Ceres project at https://github.com/dje-dev/ceres.
  Copyright (C) 2020- by David Elliott and the Ceres Authors.

  Ceres is free software under the terms of the GNU General Public License v3.0.
  You should have received a copy of the GNU General Public License
  along with Ceres. If not, see <http://www.gnu.org/licenses/>.
*/

#endregion

#region Using directives

using System.IO;
using System.Globalization;
using static System.FormattableString;

using Ceres.Chess.NNEvaluators.Specifications;
using Ceres.Chess.UserSettings;

using Ceres.Chess.NNBackends.ONNXRuntime;

using Ceres.MCGS.Managers.Limits;
using Ceres.MCGS.Search.Params;

#endregion

namespace Ceres.MCGS.UCI;

public partial class UCIManagerMCGS
{
  /// <summary>
  /// Optional text file to receive log of all UCI dialog between engine and client.
  /// </summary>
  string uciLogFileName = CeresUserSettingsManager.Settings.UCILogFile;

  /// <summary>
  /// Optional text file to receive diagnostic log relating to Ceres engine moves.
  /// </summary>
  string searchLogFileName = CeresUserSettingsManager.Settings.SearchLogFile;

  /// <summary>
  /// If verbose move stats should be output at periodic intervals.
  /// </summary>
  bool logLiveStats;

  /// <summary>
  /// If chess960 is selected enable chess960 logic.
  /// </summary>
  bool UCI_Chess960;

  /// <summary>
  ///  If graph mode (MCGS) is enabled (otherwise MCTS).
  /// </summary>
  bool MCGS = true;

  /// <summary>
  /// Mode for transposition matching (position-only or position-and-history).
  /// </summary>
  PathMode pathMode = PathMode.PositionEquivalence;

  /// <summary>
  /// If detailed top-level move info should be output at end of move selection.
  /// </summary>
  bool verboseMoveStats;

  /// <summary>
  /// If the value head score should be output as a logistic (probability of winning),
  /// otherwise output as centipawn equivalent.
  /// </summary>
  bool scoreAsQ;

  /// <summary>
  /// If the W/D/L scores should each be shown in UCI info lines
  /// </summary>
  bool showWDL = true;

  /// <summary>
  /// If MultiPV info lines use N for child move.
  /// </summary>
  bool perPVCounters;

  /// <summary>
  /// Number of moves for which to output PVs in UCI input (multiPV mode is where numPV > 1).
  /// </summary>
  int numPV = 1;

  /// <summary>
  /// ParamsSelect.CPUCT
  /// </summary>
  float cpuct = new ParamsSelect().CPUCT;

  /// <summary>
  /// ParamsSelect.CPUCTBase
  /// </summary>
  float cpuctBase = new ParamsSelect().CPUCTBase;

  /// <summary>
  /// ParamsSelect.CPUCTFactor
  /// </summary>
  float cpuctFactor = new ParamsSelect().CPUCTFactor;

  /// <summary>
  /// ParamsSelect.CPUCTAtRoot
  /// </summary>
  float cpuctAtRoot = new ParamsSelect().CPUCTAtRoot;

  /// <summary>
  /// ParamsSelect.CPUCTBaseAtRoot
  /// </summary>
  float cpuctBaseAtRoot = new ParamsSelect().CPUCTBaseAtRoot;

  /// <summary>
  /// ParamsSelect.CPUCTFactorAtRoot
  /// </summary>
  float cpuctFactorAtRoot = new ParamsSelect().CPUCTFactorAtRoot;

  /// <summary>
  /// ParamsSearch.ValueTemperature
  /// </summary>
  float valueTemperature = new ParamsSearch().ValueTemperature;

  /// <summary>
  /// ParamsSelect.PolicySoftmax
  /// </summary>
  float policySoftmax = new ParamsSelect().PolicySoftmax;

  /// <summary>
  /// If futility pruning (smart pruning) is disabled.
  /// </summary>
  bool futilityPruningDisabled = new ParamsSearch().FutilityPruningStopSearchEnabled == false;

  /// <summary>
  /// Amount of seconds subtracted from time limits to compensate for overhead/latency.
  /// </summary>
  float moveOverheadSeconds = new ParamsSearch().MoveOverheadSeconds;

  /// <summary>
  ///  Path to tablebases.
  /// </summary>
  string tablebaseDirectory = CeresUserSettingsManager.Settings.SyzygyPath;

  /// <summary>
  /// Maximum size of tree allowed (in visits).
  /// </summary>
  int? maxTreeVisits;

  /// <summary>
  /// Maximum size of tree allowed (in nodes).
  /// </summary>
  int? maxTreeNodes = CeresUserSettingsManager.Settings.MaxTreeNodes;

  /// <summary>
  /// Multiplier applied to all requested time limits.
  /// </summary>
  float searchLimitMultiplier = 1.0f;

  /// <summary>
  /// Maximum RAM (in MB) that the engine should use, 0 for no limit.
  /// </summary>
  float ramLimitMb = 0;

  /// <summary>
  /// First play urgency coefficient.
  /// </summary>
  float fpu = new ParamsSelect().FPUValue;

  /// <summary>
  /// First play urgency (at root) coefficient.
  /// </summary>
  float fpuAtRoot = new ParamsSelect().FPUValueAtRoot;

  /// <summary>
  /// Set of batch sizes for which CUDA graphs are created.
  /// </summary>
  int[] cudaGraphSizes = [16, 32, 48, 64, 80];


  void ProcessSetOption(string command)
  {
    string[] parts = command.Split(" ");

    if (parts.Length < 4
     || parts[0].ToLower() != "setoption"
     || parts[1].ToLower() != "name"
     || parts[3].ToLower() != "value"
     )
    {
      OutStream.WriteLine("Expected command of form setoption name <option_name> value <option_value>");
      return;
    }

    string name = parts[2];
    string value = command.Substring(command.IndexOf("value ") + 6);

    switch (name.ToLower())
    {
      case "weightsfile":
      case "network":
        ProcessWeightsFile(value);
        break;

      case "device":
        ProcessDevice(value);
        break;

      case "searchlogfile":
        searchLogFileName = value == "" ? null : value;
        if (CeresEngine != null) CeresEngine.SearchLogFileName = searchLogFileName;
        break;

      case "logfile":
        uciLogFileName = value == "" ? null : value;
        if (uciLogFileName == null)
        {
          uciLogWriter = null;
        }
        else
        {
          uciLogWriter = new StreamWriter(new FileStream(uciLogFileName, FileMode.Append, FileAccess.Write));
        }
        break;

      case "loglivestats":
        SetBool(value, ref logLiveStats);
        break;

      case "uci_chess960":
        SetBool(value, ref UCI_Chess960);
        IsChess960OptionSet = UCI_Chess960;          
        break;

      case "mcgs":
        SetBool(value, ref MCGS);
        break;

      case "pathmode":
        SetPathMode(value);
        break;

      case "maxtreevisits":
        SetInt(value, 1, int.MaxValue, ref maxTreeVisits);
        break;

      case "maxtreenodes":
        SetInt(value, 1, int.MaxValue, ref maxTreeNodes);
        break;

      case "searchlimitmultiplier":
        SetFloat(value, 0.001f, 1_000_000, ref searchLimitMultiplier, true);
        break;

      case "ramlimitmb":
        SetFloat(value, 0.0f, 1E15f, ref ramLimitMb, true);
        break;

      case "moveoverheadms":
        float moveOverheadMilliseconds = 0;
        SetFloat(value, 0, int.MaxValue, ref moveOverheadMilliseconds, true);
        moveOverheadSeconds = moveOverheadMilliseconds / 1000f;
        break;

      case "smartpruningfactor":
        float factor = -1;
        SetFloat(value, 0, int.MaxValue, ref factor, true);
        if (factor == 0)
        {
          futilityPruningDisabled = true;
        }
        else if (factor == ParamsSearch.LC0_DEFAULT_SMART_PRUNING_FACTOR)
        {
          futilityPruningDisabled = false;
        }
        else
        {
          OutStream.Write($"Ceres does not support {factor}, only value 0 (indicating turned off) or 1.33 (indicating default) for SmartPruningFactor");
        }

        break;

      case "verbosemovestats":
        SetBool(value, ref verboseMoveStats);
        break;

      case "scoretype":
        // Lc0 supports: centipawn, centipawn_with_drawscore, centipawn_2019, centipawn_2018, win_percentage, Q, W-L, WDL_mu
        value = value.ToLower();
        if (value == "centipawn")
        {
          scoreAsQ = false;
        }
        else if (value == "w-l" || value == "q")
        {
          scoreAsQ = true;
        }
        else
        {
          OutStream.Write("Invalid value for ScoreType, allowable values are Centipawn, Q or W-L");
        }

        break;

      case "limitsmanagername":
        CeresUserSettingsManager.Settings.LimitsManagerName = value;
        if (CeresEngine != null)
        {
          CeresEngine.GameLimitManager = new ManagerGameLimitTest(CeresEngine.SearchParams.GameLimitUsageAggressiveness);
        }
        break;

      //option name ScoreType type combo default centipawn var centipawn var Q var W-L

      case "multipv":
        SetInt(value, 1, int.MaxValue, ref numPV);
        break;

      case "perpvcounters":
        SetBool(value, ref perPVCounters);
        break;

      case "uci_showwdl":
        SetBool(value, ref showWDL);
        break;

      case "reducedmemorymode":
        bool reducedMemory = false;
        SetBool(value, ref reducedMemory);
        CeresUserSettingsManager.Settings.ReducedMemoryMode = reducedMemory;
        break;

      case "syzygypath":
        tablebaseDirectory = value;
        break;

      case "cpuct":
        SetFloat(value, 0, float.MaxValue, ref cpuct, true);
        break;

      case "cpuctbase":
        SetFloat(value, 1, float.MaxValue, ref cpuctBase, true);
        break;

      case "cpuctfactor":
        SetFloat(value, 0, float.MaxValue, ref cpuctFactor, true);
        break;

      case "cpuctatroot":
        SetFloat(value, 0, float.MaxValue, ref cpuctAtRoot, true);
        break;

      case "cpuctbaseatroot":
        SetFloat(value, 1, float.MaxValue, ref cpuctBaseAtRoot, true);
        break;

      case "cpuctfactoratroot":
        SetFloat(value, 0, float.MaxValue, ref cpuctFactorAtRoot, true);
        break;

      case "policytemperature":
        SetFloat(value, 0.1f, float.MaxValue, ref policySoftmax, true);
        break;

      case "valuetemperature":
        SetFloat(value, 0.01f, float.MaxValue, ref valueTemperature, true);
        break;

      case "fpu":
        SetFloat(value, 0, float.MaxValue, ref fpu, true);
        break;

      case "fpuatroot":
        SetFloat(value, 0, float.MaxValue, ref fpuAtRoot, true);
        break;

      case "cudagraphsizes":
        SetCudagraphBatchSizes(value);
        break;
    }
  }

  private void SetCudagraphBatchSizes(string value)
  {
    // Remove whitespace
    value = value.Trim();

    // Check for array format [...]
    if (!value.StartsWith("[") || !value.EndsWith("]"))
    {
      OutStream.WriteLine("Invalid format for CudagraphSizes. Expected format: [14,28,44,64]");
      return;
    }

    // Remove brackets and split by comma
    string innerValue = value.Substring(1, value.Length - 2).Trim();
    
    if (string.IsNullOrEmpty(innerValue))
    {
      OutStream.WriteLine("Invalid format for CudagraphSizes. Expected at least one batch size.");
      return;
    }

    string[] parts = innerValue.Split(',');
    int[] newBatchSizes = new int[parts.Length];
    
    // Parse and validate each value
    for (int i = 0; i < parts.Length; i++)
    {
      string part = parts[i].Trim();
      
      if (!int.TryParse(part, out int batchSize))
      {
        OutStream.WriteLine($"Invalid value '{part}' in CudagraphSizes. Expected positive integer.");
        return;
      }

      if (batchSize <= 0)
      {
        OutStream.WriteLine($"Invalid value {batchSize} in CudagraphSizes. Expected positive integer.");
        return;
      }

      newBatchSizes[i] = batchSize;
    }

    // Validate ascending order
    for (int i = 1; i < newBatchSizes.Length; i++)
    {
      if (newBatchSizes[i] <= newBatchSizes[i - 1])
      {
        OutStream.WriteLine($"Invalid CudagraphSizes. Values must be in ascending order. Found {newBatchSizes[i - 1]} followed by {newBatchSizes[i]}.");
        return;
      }
    }

    // All validation passed, set the new value
    cudaGraphSizes = newBatchSizes;

    ONNXExecutor.OVERRIDE_CUDA_GRAPH_BATCH_SIZES = cudaGraphSizes;
    ReinitializeEngine();
    if (NetworkSpec != null)
    {
      CreateEvaluator();
    }
  }


  private void ProcessWeightsFile(string value)
  {
    if (taskSearchCurrentlyExecuting != null)
    {
      OutStream.WriteLine("uci info Implementation limitation: cannot change weights while search is running.");
    }
    else
    {
      if (value == null || value == "")
      {
        OutStream.WriteLine("uci info Network file name expected");
      }
      else
      {
        if (EvaluatorDef != null && EvaluatorDef.Nets[0].Net.NetworkID.ToLower() == value.ToLower())
        {
          OutStream.WriteLine($"uci info Specified network file is already {value}, ignoring.");
        }
        else
        {
          ReinitializeEngine();
          string netIDToUse = value.Replace("\"", null).Replace("'", null);
          NetworkSpec = new NNNetSpecificationString(netIDToUse);

          if (NetworkSpec.AutodownloadCeresNetIfNeeded(false))
          {
            CreateEvaluator();
          }
        }
      }
    }
  }

  private void ProcessDevice(string value)
  {
    if (taskSearchCurrentlyExecuting != null)
    {
      OutStream.WriteLine("uci info Implementation limitation: cannot change device while search is running.");
    }
    else
    {
      if (value == null || value == "")
      {
        OutStream.WriteLine("uci info device specification expected (e.g. \"GPU:0\")");
      }
      else
      {
        ReinitializeEngine();
        DeviceSpec = new NNDevicesSpecificationString(value.Replace("\"", null).Replace("'", null));
        if (NetworkSpec != null)
        {
          CreateEvaluator();
        }
      }
    }
  }



  void SetBool(string boolStr, ref bool value, bool reinitializeEngine = true)
  {
    if (boolStr.ToLower() == "true")
    {
      value = true;
    }
    else if (boolStr.ToLower() == "false")
    {
      value = false;
    }
    else
    {
      OutStream.WriteLine("Invalid value, expected true or false");
    }

    if (CeresEngine != null)
    {
      ReinitializeEngine();
    }
  }

  void SetPathMode(string pathModeStr)
  {
    string normalizedValue = pathModeStr.ToLower();
    if (normalizedValue == "position")
    {
      pathMode = PathMode.PositionEquivalence;
    }
    else if (normalizedValue == "positionandhistory")
    {
      pathMode = PathMode.PositionAndHistoryEquivalence;
    }
    else
    {
      OutStream.WriteLine("Invalid value for PathMode, expected Position or PositionAndHistory");
    }

    if (CeresEngine != null)
    {
      ReinitializeEngine();
    }
  }


  void SetInt(string intStr, int minValue, int maxValue, ref int? value)
  {
    int setValue = 0;
    SetInt(intStr, minValue, maxValue, ref setValue);
    value = setValue;
  }

  void SetInt(string intStr, int minValue, int maxValue, ref int value)
  {
    if (!int.TryParse(intStr, out int newValue))
    {
      OutStream.WriteLine("Invalid value, expected integer");
    }

    if (newValue < minValue)
    {
      OutStream.WriteLine($"Value below minimum of {minValue}");
    }
    else if (newValue > maxValue)
    {
      OutStream.WriteLine($"Value above maximum of {maxValue}");
    }
    else
    {
      value = newValue;
    }
  }

  void SetFloat(string floatStr, float minValue, float maxValue, ref float value, bool reinitializeEngine)
  {
    if (!float.TryParse(floatStr, NumberStyles.Any, CultureInfo.InvariantCulture, out float newValue))
    {
      OutStream.WriteLine("Invalid value, expected number");
    }

    if (newValue < minValue)
    {
      OutStream.WriteLine($"Value below minimum of {minValue}");
    }
    else if (newValue > maxValue)
    {
      OutStream.WriteLine($"Value above maximum of {maxValue}");
    }
    else
    {
      value = newValue;
    }

    if (reinitializeEngine && CeresEngine != null)
    {
      ReinitializeEngine();
    } 
  }

  static string SetOptionUCIDescriptions => Invariant(
@$"
option name Device type string default
option name WeightsFile type string default
option name SyzygyPath type string default
option name LogFile type string default
option name SearchLogFile type string default
option name MultiPV type spin default 1 min 1 max 500
option name VerboseMoveStats type check default false
option name LogLiveStats type check default false
option name UCI_Chess960 type check default false
option name SmartPruningFactor type string default 1.33
option name MoveOverheadMs type spin default {new ParamsSearch().MoveOverheadSeconds * 1000} min 0 max 100000000
option name RamLimitMb type spin default 0 min 0 max 999999999 default 0
option name PerPVCounters type check default false
option name ScoreType type combo default centipawn var centipawn var Q var W-L
option name UCI_ShowWDL type check default false
option name MCGS type check default true
option name PathMode type combo default Position var Position var PositionAndHistory
option name CPUCT type string default {new ParamsSelect().CPUCT}
option name CPUCTAtRoot type string default {new ParamsSelect().CPUCTAtRoot}
option name CPUCTBase type string default {new ParamsSelect().CPUCTBase}
option name CPUCTBaseAtRoot type string default {new ParamsSelect().CPUCTBaseAtRoot}
option name CPUCTFactor type string default {new ParamsSelect().CPUCTFactor}
option name CPUCTFactorAtRoot type string default {new ParamsSelect().CPUCTFactorAtRoot}
option name PolicyTemperature type string default {new ParamsSelect().PolicySoftmax}
option name ValueTemperature type string default {new ParamsSearch().ValueTemperature}
option name FPU type string default {new ParamsSelect().FPUValue}
option name FPUAtRoot type string default {new ParamsSelect().FPUValueAtRoot}
option name SearchLimitMultiplier type string default 1.00
option name MaxTreeVisits type string default
option name MaxTreeNodes type string default
option name ReducedMemoryMode type check default false
option name EnableSiblingEval type check default false
option name EnableUncertaintyBoosting type check default false
option name LimitsManagerName type string default
option name Network type string default
option name CUDAGraphSizes type string default [14,28,44,64]
");
  /* 
option name ConfigFile type string default lc0.config
option name HistoryFill type combo default fen_only var no var fen_only var always
";
*/
}