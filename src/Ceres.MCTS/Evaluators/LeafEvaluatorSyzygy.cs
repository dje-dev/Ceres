    ...
    internal LeafEvaluationResult Lookup(in Position pos)
    {
      ...
      switch (score)
      {
        ...
        case SyzygyWDLScore.WDLDraw:
          result = new LeafEvaluationResult(GameResult.Draw, 0, 0, -1, 0, 0, 0);
          break;

        case SyzygyWDLScore.WDLWin:
          result = new LeafEvaluationResult(GameResult.Checkmate, (FP16)ParamsSelect.WinPForProvenWin(1, false), 0, 1, 1, 0, 0);
          break;

        case SyzygyWDLScore.WDLLoss:
          result = new LeafEvaluationResult(GameResult.Checkmate, 0, (FP16)ParamsSelect.LossPForProvenLoss(1, false), 1, 0, 0, 0);
          break;

        case SyzygyWDLScore.WDLCursedWin:
          // Score as almost draw, just slightly positive (since opponent might err in obtaining draw)
          result = new LeafEvaluationResult(GameResult.Draw, (FP16)0.05f, 0, 100, 0, 0, 0);
          break;

        case SyzygyWDLScore.WDLBlessedLoss:
          // Score as almost draw, just slightly negative (since we might err in obtaining draw)
          result = new LeafEvaluationResult(GameResult.Draw, 0, (FP16)0.05f, 100, 0, 0, 0);
          break;
