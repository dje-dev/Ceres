...
*/
  ...
  public partial class LeafEvaluatorTransposition
  {
    ...
    public static void InsurePendingTranspositionValuesSet(MCTSNode node, bool possiblyRefresh)
    {
    ...
    static void SetPendingTransitionValues(MCTSNode node,
                                           ...
                                           bool forceUseV)
    {
      if (node.Context.ParamsSearch.TranspositionUseTransposedQ && !forceUseV)
      {
      ...
      {
        // Not expected to find null eval result if not yet initialized.
        // (though we handle that case below just in case).
        Debug.Assert(node.N > 0 || !node.EvalResult.IsNull);

        if (node.N == 0 && !node.EvalResult.IsNull)
        {
          node.PendingTranspositionV = node.EvalResult.V;
          node.PendingTranspositionM = node.EvalResult.M;
          node.PendingTranspositionD = node.EvalResult.DrawP;
        }
        else
        {
          node.PendingTranspositionV = node.V;
          node.PendingTranspositionM = node.MPosition;
          node.PendingTranspositionD = node.DrawP;
        }

        if (forceUseV)
        {
          // Make sure we don't keep repeating a V value.
          node.Ref.NumVisitsPendingTranspositionRootExtraction = Math.Min(1, node.NumVisitsPendingTranspositionRootExtraction);

          if (node.Context.ParamsSearch.MaxTranspositionRootApplicationsFixed > 1)
          {
    ...
    static bool ForceUseV(MCTSNode node, in MCTSNodeStruct transpositionRootNode)
    {
      ...
      return false;
      const int MAX_EXTRA_TRANSPOSITION_ROOT_VISITS = 3;

      int MAX_ROOT_N = MAX_EXTRA_TRANSPOSITION_ROOT_VISITS
                     + node.Context.ParamsSearch.MaxTranspositionRootApplicationsFixed;
      return transpositionRootNode.N > MAX_ROOT_N;
    }
