...
namespace Ceres.MCTS.LeafExpansion
{
  ...
  public class MCTSTree
  {
                    ...
                    PositionEvalCache positionCache, IMCTSNodeCache reuseNodeCache)
    {
      ...
      {
        EncodedPositionBoard thisBoard = EncodedPositionBoard.FromPosition(in priorPositions[i], priorPositions[i].MiscInfo.SideToMove);
    ...
    public unsafe void Annotate(MCTSNode node, bool forceAnnotate = false)
    {
      ...
      if (!FAST)
      {
        annotation.LC0BoardPosition = EncodedPositionBoard.FromPosition(in posHistory[^1], annotation.PosMG.SideToMove);
      }
      else
      {
      ...
      if (alreadyEvaluated)
      {
        ...
        node.EvalResult.Initialize(node.Terminal, node.WinP, node.LossP, node.MPosition, 
                                   node.VSecondary, node.UncertaintyVPosition, node.UncertaintyPPosition);
