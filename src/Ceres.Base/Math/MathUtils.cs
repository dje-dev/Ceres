...
public static class MathUtils
{
  ...
  /// <summary>
  /// Fast ln(x) approximation for x > 0 using float bit decomposition
  /// and a 5th-degree polynomial on [1,2).
  /// Approximately 2x to 3x faster than MathF.Log.
  /// Maximum approximation error (around 1) is about 0.1%.
  /// </summary>
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static float FastLog(float x)
  {
    ...
    // Force exponent = 127 --> mantissa in [1,2)
    bits = (bits & 0x7FFFFF) | (127 << 23);
    float m = BitConverter.Int32BitsToSingle(bits);
    float t = m - 1.0f;          // t in [0,1)

    // ln(1 + t) approx Sum(c_k t^k), minimax-ish fit on [0,1]
    const float C1 = 0.9993986f;
    const float C2 = -0.4912327f;
    const float C3 = 0.2879880f;
    const float C4 = -0.1348161f;
    const float C5 = 0.0318295f;

    // Horner: ln(1+t) approx ( ( ( (C5 t + C4) t + C3) t + C2) t + C1 ) * t
    float p = (((C5 * t + C4) * t + C3) * t + C2) * t + C1;
    float ln_m = p * t;

    const float LN2 = 0.69314718056f;
    return exponent * LN2 + ln_m;
  }
