...
namespace Ceres.Base.DataTypes.TFRecord
{
	...
	internal class Crc32CAlgorithm : HashAlgorithm
	{
		...
		internal Crc32CAlgorithm(bool isBigEndian = true) : this()
		{
		...
		internal static uint Append(uint initial, byte[] input, int offset, int length)
		{
			if (input == null)
		...
		/// <returns>Accumulated CRC-32C of all buffers processed so far.</returns>
		internal static uint Append(uint initial, byte[] input)
		{
		...
		internal static uint Compute(byte[] input, int offset, int length)
		{
		...
		/// <param name="length">Length of the input data in the buffer.</param>
		/// <returns>CRC-32C of the data in the buffer.</returns>
		internal static uint ComputeAndWriteToEnd(byte[] input, int offset, int length)
		{
			if (length + 4 > input.Length) throw new ArgumentOutOfRangeException("length", "Length of data should be less than array length - 4 bytes of CRC data");
      ...
      int r = offset + length;
		...
		/// <param name="input">Input buffer with data to be checksummed.</param>
		/// <returns>CRC-32C of the data in the buffer.</returns>
		internal static uint ComputeAndWriteToEnd(byte[] input)
		{
			if (input.Length < 4)	throw new ArgumentOutOfRangeException("input", "Input array should be 4 bytes at least");
		...
		internal static bool IsValidWithCrcAtEnd(byte[] input, int offset, int lengthWithCrc)
		{
		...
		/// <param name="input">Input buffer with data to be checksummed.</param>
		/// <returns>Is checksum valid.</returns>
		internal static bool IsValidWithCrcAtEnd(byte[] input)
		{
			if (input.Length < 4) throw new ArgumentOutOfRangeException("input", "Input array should be 4 bytes at least");
