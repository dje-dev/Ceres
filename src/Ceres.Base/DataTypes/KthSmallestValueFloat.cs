...
namespace Ceres.Base.DataTypes
{
  ...
  /// via partitioning.
  /// </summary>
  public static class KthSmallestValueFloat
  {
    /// <summary>
    /// 
    ...
    /// <param name="k"></param>
    /// <returns></returns>
    public static float CalcKthSmallestValue(float[] values, int k)
    {
    ...
    /// <param name="k"></param>
    /// <returns></returns>
    static float Partition(float[] values, int minIndex, int maxIndex, int k)
    {
      if (minIndex >= maxIndex) return values[minIndex];

      float pivotValue = values[minIndex];
      int pivotIndex = minIndex;
      for (int i = minIndex + 1; i <= maxIndex; i++)
      {
        if (values[i] >= pivotValue)
          ArrayUtils.SwapStructsInArray(values, ++minIndex, i);
      }

      ArrayUtils.SwapStructsInArray(values, pivotIndex, minIndex);
