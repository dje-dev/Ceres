...
namespace Ceres.Chess.Textual
{
  ...
  public static class SANParser
  {
    ...
    /// <returns></returns>
    static (bool, Square) PawnOnFileBeforeSquare(Square square, in Position pos)
    {
    ...
    static (bool, Square) SquareWithPieceThatCanMoveToSquare(Piece piece, Square targetSquare, Position pos, bool withCapture, string originRankOrFileOrSquare)
    {
      ...
      for (int i = 0; i < moves.NumMovesUsed; i++)
      {
        ...
        bool originRankOrFileOk = originRankOrFileOrSquare == default 
                               || thisFromSquare.ToString().Contains(originRankOrFileOrSquare);
    ...
    static bool ParsePieceThenSquare(char pieceChar, string targetSquareStr, 
                                     in Position pos, string originRankOrFileOrSquare, bool withCapture,
                                     PieceType promotionType, out Move move)
    {
      move = default;

      if (originRankOrFileOrSquare != null)
      {
        originRankOrFileOrSquare = originRankOrFileOrSquare.ToUpperInvariant();
      }
    ...
    static Move ParseNormalSANMove(string str, in Position pos)
    {
      string orgStr = str;
      ...
      {
        (bool found, Square square) = PawnOnFileBeforeSquare(str, in pos);
        if (!found)
        {
          throw new ArgumentException($"No eligible pawn found in SAN {str}");
        }
        return new Move(square, str, promoPiece);
      }
      else if (str.Length == 3) // e.g. Nf3
      {
        if (ParsePieceThenSquare(str[0], str.Substring(1), in pos, null, false, promoPiece, out Move retMove))
        {
          return retMove;
        }
      }
      else if (str.Length == 4 && str[1] == 'x') // e.g. Qxf6 or exc3
      {
        ...
        {
          if (ParsePieceThenSquare(str[0], str.Substring(2), in pos, null, true, promoPiece, out Move retMove))
          {
            return retMove;
          }
        }
        ...
        {
          if (ParsePieceThenSquare('P', str.Substring(2), in pos, str.Substring(0, 1), true, promoPiece, out Move retMove))
          {
            return retMove;
          }
        }
      ...
      {
        if (ParsePieceThenSquare(str[0], str.Substring(2), in pos, str.Substring(1, 1), true, promoPiece, out Move retMove))
        {
          return retMove;
        }
      }
      ...
      {
        if (ParsePieceThenSquare(str[0], str.Substring(3), in pos, str.Substring(1, 1), true, promoPiece, out Move retMove))
        {
          return retMove;
        }
      }
      ...
      {
        if (ParsePieceThenSquare(str[0], str.Substring(3), in pos, str.Substring(1, 2), false, promoPiece, out Move retMove))
        {
          return retMove;
        }
      }
    ...
    public static PositionWithMove FromSAN(string sanString, in Position pos)
    {
      ...
      if (indexFirstPlus != -1)
      {
        sanString = sanString.Substring(0, indexFirstPlus);
      }
      ...
      if (indexFirstHash != -1)
      {
        sanString = sanString.Substring(0, indexFirstHash);
      }

      switch (sanString)
      {
        case null: throw new ArgumentNullException();
        case "O-O": return new PositionWithMove(in pos, new Move(Move.MoveType.MoveCastleShort));
        case "O-O-O": return new PositionWithMove(in pos, new Move(Move.MoveType.MoveCastleLong));
        case "1-0": return new PositionWithMove(in pos, new Move(Move.MoveType.WhiteWins));
        case "0-1": return new PositionWithMove(in pos, new Move(Move.MoveType.BlackWins));
        case "1/2-1/2": return new PositionWithMove(in pos, new Move(Move.MoveType.Draw));
        default: return new PositionWithMove(in pos, ParseNormalSANMove(sanString, in pos));
      }
    }
