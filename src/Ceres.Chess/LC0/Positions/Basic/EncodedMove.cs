...
{
  /// <summary>
  /// Represents a chess move, encoded in the Leela Chess Zero way.
  /// Moves are always represented with the board oriented from the perspective of the player to move.
  /// 
  /// Raw value is 16 bits as follows:
  ///   - 6 bits "to" square
  ///   - 6 bits "from square
  ...
  public readonly struct EncodedMove : IEquatable<EncodedMove>
  {
    ...
    internal const ushort MaskFrom = 0b0000111111000000; // bits [6..11]
    private const ushort MaskPromotion = 0b0111000000000000; // bits [12..14]
    private const ushort MaskCastling = 0b1000000000000000; // bit 15

    private const ushort MaskFlip = 0b111000111000;
    ...
    public EncodedMove(ushort value) => rawValue = value;


    /// <summary>
    /// Constructor (from squares).
    /// </summary>
    ...
    /// <summary>
    /// Encoding which is similar to the raw value but
    /// is compressed into 14 bits (0..16383) by:
    ...
    public EncodedSquare FromSquare => new EncodedSquare(((rawValue & MaskFrom) >> 6));

    public EncodedSquare ToSquare => new EncodedSquare(rawValue & MaskTo);
    ...
    /// <summary>
    /// Returns another EncodedMove which is flipped.
    /// </summary>
    public EncodedMove Flipped => new EncodedMove((ushort)(rawValue ^ MaskFlip));

    public override int GetHashCode() => rawValue;

    /// <summary>
    /// Returns another EncodedMove which is mirrored.
    /// </summary>
    public EncodedMove Mirrored
    {
      get
      {
    ...
    /// <summary>
    /// Equality is based on IndexPacked, and thus does not consult the promotion and castling flags.
    /// This is because the setting of these flags are optional (uniquely specifies move in a given position).
    /// </summary>
    /// <param name="other"></param>
    /// <returns></returns>
    public bool Equals(EncodedMove other) => this.IndexPacked == other.IndexPacked;
    ...
    public static bool operator ==(EncodedMove m1, EncodedMove m2) => m1.IndexPacked == m2.IndexPacked;
    public static bool operator !=(EncodedMove m1, EncodedMove m2) => m1.IndexPacked != m2.IndexPacked;
    ...
    {
      ushort[] temp = new ushort[64 * 64 * 4];
    ...
    /// <summary>
    /// Factory method from neural net index (when information was not available about the piece moving to help fill in promotion castling flags)
    /// </summary>
    /// <param name="index"></param>
    /// <returns></returns>
    public static EncodedMove FromNeuralNetIndex(int index) => FromNeuralNetIndex(index, false, false, false);


    /// <summary>
    /// Factory method from neural net index (when information was available about the piece moving to help fill in promotion castling flags)
    /// </summary>
    /// <param name="index"></param>
    /// <param name="pieceInfoWasAvailable"></param>
    /// <param name="isPawnMove"></param>
    /// <param name="isKingMove"></param>
    /// <returns></returns>
    ...
    static EncodedMove FromNeuralNetIndex(int index, bool pieceInfoWasAvailable, bool isPawnMove, bool isKingMove)
    {
      ...
      if (pieceInfoWasAvailable)
      {
        ...
        else if (isPawnMove && raw.ToSquare.Rank == 7) // promotion
        {
          ...
          int promoIndex = Array.IndexOf(PROMO_CHARS, possiblePromoChar);
    ...
    static readonly string[] NEURAL_NET_MOVE_STR = new string[] {
